...
function addToDo(event) {
    ...
    let toDoList = rawData ? JSON.parse(rawData) : [];
...
function getData() {
    ...
    if (!toDoContainer)
        return;
    const rawData = localStorage.getItem('myToDos');
    const toDoList = rawData ? JSON.parse(rawData) : [];
...
// Since TS keeps code isolated, inline HTML attributes need explicit window exposure
